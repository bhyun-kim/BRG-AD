...
from utils.visualization.plot_images_grid import plot_images_grid, plot_signal_list
from model.DeepSAD import DeepSAD
from datasets.main import load_dataset


"""
Code Reference : 
https://github.com/lukasruff/Deep-SAD-PyTorch/blob/master/src/main.py
"""
...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['hanBridge', 'bridgeFEM', 'shearBuilding', 'pscBeam']))
@click.argument('net_name', type=click.Choice(['base_CNN_1D_Autoencoder', 'base_CNN_2D_Autoencoder', 'Residual_CNN_1D_Autoencoder', 'fullyConv_1D_Autoencoder', 'fullyConv_2D_Autoencoder']))
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, eta,
    ...
    if dataset_name in ('bridgeFEM'):
        X_all_low = [dataset.test_set.read_data(x) for x in idx_all_sorted[:32]]
        X_all_high = [dataset.test_set.read_data(x) for x in idx_all_sorted[-32:]]
        X_normal_low = [dataset.test_set.read_data(x) for x in idx_normal_sorted[:32]]
        X_normal_high = [dataset.test_set.read_data(x) for x in idx_normal_sorted[-32:]]

        plot_signal_list(X_all_low, export_img=xp_path + '/all_low', padding=2)
        plot_signal_list(X_all_high, export_img=xp_path + '/all_high', padding=2)
        plot_signal_list(X_normal_low, export_img=xp_path + '/normals_low', padding=2)
        plot_signal_list(X_normal_high, export_img=xp_path + '/normals_high', padding=2)
