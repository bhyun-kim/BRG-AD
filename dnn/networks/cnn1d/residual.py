...
class Residual_CNN_1D_Encoder(BaseNet):
    def __init__(self):
        super(Residual_CNN_1D_Encoder, self).__init__()
        
        self.rep_dim = 160 
       
        #Encoder
        self.conv1 = nn.Conv1d(1, 16, 3 * 3 * 3, padding=13)  
        self.bn1 = nn.BatchNorm1d(16)

        self.conv2 = nn.Conv1d(32, 32, 3 * 3 * 3, padding=13)
        self.bn2 = nn.BatchNorm1d(32)

        self.conv3 = nn.Conv1d(64, 64, 3 * 3 * 3, padding=13)
    ...
    def forward(self, x, return_indices=False):
        ...
        x = self.conv2(x_res)
        x = self.bn2(x)
        x = torch.cat((self.act(x), x_res), dim=1)
        x_res, index = self.pool(x)
        indices.append(index)
...
class Residual_CNN_1D_Decoder(BaseNet):
    def __init__(self):
        super(Residual_CNN_1D_Decoder, self).__init__()
...
class Residual_CNN_1D_Autoencoder(BaseNet):
    def __init__(self):
        
        super(Residual_CNN_1D_Autoencoder, self).__init__()

        self.rep_dim= 160
        self.encoder = Residual_CNN_1D_Encoder()
        self.decoder = Residual_CNN_1D_Decoder()
